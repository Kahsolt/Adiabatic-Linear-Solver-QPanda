...
4. 哈密顿量演化模拟从 H0 逐渐演化到 H1，该过程即将初态 |φ> 演化为期望的末态 |x>
  - 含时哈密顿量 H(s) 被拆成小片 (Dyson series)，且指数化成酉矩阵/量子门之后，作用在量子态上进行演化
...
7. 实验设置
  - 两种配置的随机矩阵: N=16, κ≈10; N=32, κ≈50
  - 重复次数 n_rep: 50/200
  - 迭代次数 q: 0 ~ 1500
  - 精度标杆
    - N=16, κ≈10, q≈500 -> ε≈0.01
    - N=32, κ≈50, q≈1000 -> ε≈0.01
'''
...
6. 实验中运行时间 T = 0 ~ 1000, κ = 5 ~ 35
  - 精度标杆: AQC(2), κ=20, T≈200 -> ε=0.001
'''
...
2. 关于QWalk 算子 W_T(s) 的实现
  - 基于系数矩阵 A 的块编码可以构造 H(s) 的块编码，然后在辅助比特上进行反射操作就得到 W_T(s)
    - “基于A构造H(s)” 这一步在本文 Appendix E. 给出了矩阵语言描述，在 arXiv:2312.07690 中给出了逻辑门线路图
    - “反射操作” 仍然没说清楚；根据 pennylane 的定义，若酉阵 |ψ> = U|0>，则反射 R(U) 定义为
      - R(U) = −I + |ψ⟩⟨ψ| = U (−I + |0⟩⟨0|) U†
      - 则取 U 为 U_H(s) = BE(H(s)), 根据 BE 有性质 <0|BE(H)|0> = H/λ
      - W_T = R(BE(H)) = −I + H/λ, wtf!
  - 但又按照讲解视频 W_T(s) 算子的实际效果是 expm(i * arcsin(H/λ))
    - 此处 λ 是 BE(H) 中的放缩因子(的倒数)
    - 为什么引入了 arcsin 仍不清楚，也许是线路设计的某些细节产生的副作用 (?)
3. 实验中步数 T = 50000
'''
